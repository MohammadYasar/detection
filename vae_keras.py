...
class VAE:
    ...
    def getInput(self, data):
        for i in range(data.shape[0]):
            ...
            self.trainModel(vae_data, itr_num=i)
    ...
    def trainModel(self, vae_data, trial="-1", itr_num=0, gesture="g", train=0):
        ...
        batch_size = 64
        epochs = 150
        figure_path = os.path.join(self.dataPath, self.task, "experiments", "vae", "figures")
        csv_path = os.path.join(self.dataPath, self.task, "experiments",  "vae", "csvs")
        checkpoint_path = os.path.join(self.dataPath, self.task, "experiments",  "vae", "checkpoints")
        ...
        cp3 = CSVLogger(filename="{}/lstm_model_{}gesture{}.csv".format(csv_path, itr_num, gesture), append=False, separator=';')
